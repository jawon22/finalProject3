...
//				int attachNo = attachDao.sequence();
	...
	@DeleteMapping("/image/{empNo}")
	public ResponseEntity<String> deleteProfileImage(@PathVariable int empNo){
		profileDao.deleteProfileImage(empNo);
		return ResponseEntity.ok().body("프로필 이미지 삭제성공");
	}
